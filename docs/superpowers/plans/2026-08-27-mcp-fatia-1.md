...
describe("signSupabaseJwt", () => {
  it("assina um token que o segredo verifica, com as claims que a RLS lê", async () => {
    const token = await signSupabaseJwt("11111111-1111-4111-8111-111111111111")
  ...
  it("expira no prazo pedido", async () => {
    const token = await signSupabaseJwt("11111111-1111-4111-8111-111111111111", 60)
    const { payload } = await jwtVerify(token, new TextEncoder().encode(SECRET))
  ...
  it("recusa assinar sem o segredo configurado", async () => {
    ...
    await expect(signSupabaseJwt("11111111-1111-4111-8111-111111111111")).rejects.toThrow(
      /SUPABASE_JWT_SECRET/
    )
    process.env.SUPABASE_JWT_SECRET = SECRET
  })
...
- [ ] **Step 5b: Isentar `/api/mcp` do middleware de cookie**

Sem isto o handler **nunca roda**: `src/middleware.ts` barra qualquer `/api/*`
que não esteja em `publicRoutes` com um 401 próprio — cujo corpo é
`{"error":"Unauthorized"}`, idêntico ao do handler, o que mascara o sintoma.
`/api/mcp` autentica por Bearer na própria rota, igual a `/api/integration`.

Em `src/middleware.ts`, acrescentar `'/api/mcp'` ao array `publicRoutes`.
...
> **Corrigido durante a execução.** O desenho original — receber `userId` e um
> `workspaceId` alheio por argumento — tinha dois defeitos. Exigia que você
> criasse um segundo workspace na mão, e, pior, **passaria com o JWT quebrado**:
> um token de assinatura inválida também lê zero linhas, e "não vazou nada" seria
> lido como sucesso. O script implementado segue o padrão de
> `verify-workspace-rls.mjs` (cria usuários e workspace descartáveis, limpa no
> `finally`) e abre com um **controle positivo**: antes de afirmar isolamento,
> afirma que o JWT que nós assinamos é aceito pela RLS e lê dados reais. Também
> verifica que um token expirado é recusado.
...
Cinco correções, todas descobertas rodando o que o plano mandava rodar.

1. **Task 1 refeita: cunhar sessão, não assinar JWT.** O plano assumia um
   `SUPABASE_JWT_SECRET` ativo, deduzindo isso da anon key ser HS256. Errado: a
   anon key é uma credencial legada e não diz nada sobre a chave que assina
   sessões. O projeto já migrou para ECC P-256, cuja parte privada a Supabase não
   exporta. `jwt.ts` virou `session.ts` (admin `generate_link` + `verify`, com
   cache e refresh). As 14 tools não mudaram — só consomem `identity.supabase`.

2. **O middleware barrava `/api/mcp`** e devolvia um 401 de corpo idêntico ao do
   handler, o que mascarava o sintoma. Ver Step 5b da Task 4.

3. **`resolveIdentity` caía em workspace com 2+ pessoas.** O plano dizia "a RLS já
   filtraria sozinha" — mas a RLS de `workspace_members` escopa por **workspace**,
   não por usuário: um membro enxerga todos os outros. Sem filtro de `user_id` a
   consulta devolvia duas linhas e o `maybeSingle` derrubava a autenticação. Só
   aparece em workspace compartilhado, que é o caso real deste banco.

4. **O verificador do plano passaria com o token quebrado.** Ele só afirmava "não
   vazou nada" — que um token inválido também satisfaz, porque lê zero linhas. O
   script implementado abre com controle positivo.

5. **Fixtures de UUID inválidos** sob o zod 4, que checa o nibble de variante.
   O schema estava certo; os testes é que estavam.
...
- Verificado contra o banco real: `npm run verify:mcp` passa, `tools/list` devolve as 14, `pipeline_overview` e `whats_stuck` batem com o pipeline, e `log_activity` grava, deduplica e respeita a invariante do `client_engaged`.
- **Não funciona no claude.ai nem no celular, e nada foi para produção** — é isso que a Fatia 2 resolve, trocando o corpo de `resolveIdentity`.
